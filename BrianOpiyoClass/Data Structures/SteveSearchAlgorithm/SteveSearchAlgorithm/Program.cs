...
namespace SteveSearchAlgorithm
{
            ...
            if (arrMid >= 0 && arrMid < nums.Length)
            {
                ...
                else
                {
                    ...
                    foreach (var half in twoHalves)
                    {
                        ...
                        var result = BinarySearch(half.Value, target, currentMid); // Recursive call for each half

                        if (result == -1)
                            continue; // Continue the loop if target is not found in the current half
                        else
                        {
                            return result + currentMid + 1; // Return the final index by adding the current midpoint and the result index
                        }
                    }
